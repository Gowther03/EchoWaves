...
@Component({
  selector: 'app-womens-top',
  templateUrl: './womens-top.component.html',
  styleUrls: ['./womens-top.component.css']
})
export class WomensTopComponent {

WomensTopCategories: any = {
  top: [],
  };
  ...
  fetchWomensCategories(pageNumber: number, pageSize: number): void {
    this.productService.getWomenProducts("Top",pageNumber, pageSize).subscribe({
      next: (response: any) => {
        ...
        this.WomensTopCategories.top.forEach((item: any) => {
      ...
      error: (err: any) => {
        ...
        this.showToast('Error fetching womens categories.');
      }
    });
  }
  ...
  openModal(itemId: number, event: MouseEvent): void {
    const category = Object.values(this.WomensTopCategories).flat();
