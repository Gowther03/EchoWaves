...
@Component({
  selector: 'app-mens-tshirt',
  templateUrl: './mens-tshirt.component.html',
  styleUrls: ['./mens-tshirt.component.css']
})
export class MensTshirtComponent {
 ...
 mensTshirtCategories: any = {
  ...
  fetchMensCategories(pageNumber: number, pageSize: number): void {
    this.productService.getMenProducts("T-Shirt",pageNumber, pageSize).subscribe({
      next: (response: any) => {
        ...
        this.mensTshirtCategories.tshirts.forEach((item: any) => {
  ...
  openModal(itemId: number, event: MouseEvent): void {
    const category = Object.values(this.mensTshirtCategories).flat();
    ...
    if (selectedItem) {
      ...
      const modalElement = document.getElementById('mensTshirtsCategoryModal');
      if (!modalElement) {
        return;
      }
