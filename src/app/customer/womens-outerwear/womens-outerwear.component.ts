...
@Component({
  selector: 'app-womens-outerwear',
  templateUrl: './womens-outerwear.component.html',
  styleUrls: ['./womens-outerwear.component.css']
})
export class WomensOuterwearComponent {
WomensOuterWareCategories: any = {
  ...
  fetchWomensCategories(pageNumber: number, pageSize: number): void {
    this.productService.getWomenProducts("Outerwear",pageNumber, pageSize).subscribe({
      next: (response: any) => {
        ...
        this.WomensOuterWareCategories.outerwear.forEach((item: any) => {
      ...
      error: (err: any) => {
        ...
        this.showToast('Error fetching womens outerwear categories. Please try again later.');
      }
    });
  }
